...
Create a `riku.config.js` and an `index.js` file.

<span class="tiny">**Important:** You need a <u>Application ID</u> and <u>Token</u></span>
...
```js index.js
const { bot } = require("riku");
const config = require("./riku.config.js");

module.exports = new bot(config.discord);
```
