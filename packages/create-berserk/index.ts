  ...
  if (!projectPath) {
    console.log(
      '\nPlease specify the project directory:\n' +
        ...
        'For example:\n' +
        `  ${chalk.cyan(program.name())} ${chalk.green('my-berserk-app')}\n\n` +
...
async function notifyUpdate(): Promise<void> {
  try {
    ...
    if (res?.latest) {
      const updateMessage =
        packageManager === 'yarn'
          ? 'yarn global add create-berserk-app'
          : packageManager === 'pnpm'
          ? 'pnpm add -g create-berserk-app'
          : 'npm i -g create-berserk-app'

      console.log(
        chalk.yellow.bold(
          'A new version of `create-berserk-app` is available!'
        ) +
          '\n' +
