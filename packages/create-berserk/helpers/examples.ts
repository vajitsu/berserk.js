...
export function existsInRepo(nameOrUrl: string): Promise<boolean> {
  try {
    ...
    return isUrlOk(
      `https://api.github.com/repos/vajitsu/berserk.js/contents/examples/${encodeURIComponent(
        nameOrUrl
      )}`
    )
  }
...
async function downloadTar(url: string) {
  const tempFile = join(tmpdir(), `berserk.js-cna-example.temp-${Date.now()}`)
...
export async function downloadAndExtractExample(root: string, name: string) {
  ...
  const tempFile = await downloadTar(
    'https://codeload.github.com/vajitsu/berserk.js/tar.gz/canary'
  )
  ...
  await tar.x({
    ...
    cwd: root,
    strip: 3,
    filter: (p) => p.includes(`berserk.js-canary/examples/${name}/`),
  })
