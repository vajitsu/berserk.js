...
export default class commandManager {
    ...
    public async run(interaction: Discord.ChatInputCommandInteraction & GuildInteraction) {
        if (
            !interaction.user ||
        ...
        const channelPermissions = !channel.isDMBased() ?
            channel.permissionsFor(String(this.instance.client.user?.id)) || new Discord.PermissionsBitField() :
            null;
        
        const missing = channelPermissions ? (requiredPermissions.bitfield as any) & ~(channelPermissions.bitfield as any) : null;
        ...
        (async () => {
            try {
                const data = await rest.put(
                    Discord.Routes.applicationCommands(this.instance.config.application.id),
                    { body: commands },
