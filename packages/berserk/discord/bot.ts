...
import { Client, ClientOptions } from 'berserk/dist/compiled/discord.js'
...
export default class bot {
  ...
  constructor(
    ...
    if (config.options == null)
      throw new Error(
        "> Couldn't find `discord.options`. Add a value for `options` to your `berserk.config.js` configuration under `discord`."
      )
  ...
  public async init() {
    try {
      if (
        ...
        throw new Error(
          "> Couldn't find `discord.token`. Add a value for `token` to your `berserk.config.js` configuration under `discord`."
        )
