...
import { platformArchTriples } from 'berserk/dist/compiled/@napi-rs/triples'
import * as Log from '../output/log'
import { patchIncorrectLockfile } from '../../lib/patch-incorrect-lockfile'
import { downloadWasmdiscord } from '../../lib/download-wasm-discord'
// eslint-disable-next-line import/no-extraneous-dependencies
import { version as nextVersion } from 'berserk/package.json'
...
async function tryLoadWasmWithFallback(attempts) {
  ...
  try {
    ...
    const wasmDirectory = path.join(
      path.dirname(require.resolve('berserk/package.json')),
      'wasm'
    )
...
async function loadWasm(importPath = '') {
  ...
  let attempts = []
  for (let pkg of [
    '@berserk/discord-wasm-nodejs',
    '@berserk/discord-wasm-web',
  ]) {
    try {
      let pkgPath = pkg
      ...
      if (pkg === '@berserk/discord-wasm-web') {
        bindings = await bindings.default()
      }
      Log.info('Using wasm build of berserk-discord')
      ...
      wasmBindings = {
        ...
        transform(src, options) {
          // TODO: we can remove fallback to sync interface once new stable version of berserk-discord gets published (current v12.2)
...
function loadNative() {
  ...
  for (const triple of triples) {
    try {
      bindings = require(`@berserk/discord/native/berserk-discord.${triple.platformArchABI}.node`)
      Log.info('Using locally built binary of @berserk/discord')
      break
    } catch (e) {}
  }
  ...
  if (!bindings) {
    for (const triple of triples) {
      let pkg = `@berserk/discord-${triple.platformArchABI}`
      try {
